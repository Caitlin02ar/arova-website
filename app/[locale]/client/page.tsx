...
const locations = [
    ...
    // Split description: bold part vs normal part
    const fullText = t("description");
    const boldPart = "With over 10,000 kilograms supplied across Sulawesi,";
    const restPart = fullText.replace(boldPart, "").trim();

    return (
        <main className="bg-[var(--color-cream-1)] text-[var(--color-dark-green)]">

        <section className="flex flex-col items-center justify-center px-6 pt-40 pb-4">
            <div ref={textRef} className="text-center text-2xl md:text-[1.75rem] font-heading leading-snug max-w-3xl">
            <motion.span
                className="font-bold"
        ...
        <section className="relative flex items-center justify-center px-6 py-12 md:py-16">
            ...
            {locations.map((loc, i) => (
                ...
                <motion.div
                    ...
                    style={{
                        y: "-50%",
                        color: "var(--color-dark-green)", // atau warna lain sesuai design
                    }}
